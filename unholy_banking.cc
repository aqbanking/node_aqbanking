...
int AB_create(void) {
  ...
  ab = AB_Banking_new("unholy_banking", 0, AB_BANKING_EXTENSION_NONE);
  ...
  GWEN_Gui_SetGetPasswordFn(gui, CUSTOM_GETPASSWORD_FN);
  // TODO fix me
  //GWEN_Gui_SetCheckCertFn(gui, CUSTOM_CHECKCERT_FN);
...
void AB_transactions(const v8::FunctionCallbackInfo<Value>& args) {
  ...
  if (accs) {
    ...
    if (it) {
      ...
      a=AB_Account_List2Iterator_Data(it);
      while(a) {
        //cout << "return val: " << Transactions(a) << endl;
        Transactions(a);
        a = AB_Account_List2Iterator_Next(it);
      }
      AB_Account_List2Iterator_free(it);
  ...
  //AB_ACCOUNT * a = AB_Banking_GetAccountByCodeAndNumber(ab, bank_code, account_no);
  //if (!a) {
  //  cout << "Couldn't find account (AB_Banking_GetAccountByCodeAndNumber)." << endl;
  //  return 2;
  //}
